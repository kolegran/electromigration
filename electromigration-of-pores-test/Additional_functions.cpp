    ...
    for (int x = 0; x < 2*NX; x += 2) {
        for (int y = 0; y < 2*NY; y += 2) {
            for (int z = 0; z < 2*NZ; z += 2) {
                ...
                if (NX <= x && x <= NX+SIZEX && NX <= y && y <= NY+SIZEY && NZ <= z && z <= NZ+SIZEZ) {
                    L[x  ][y  ][z  ].C_V = 0.0001;
                    L[x+1][y+1][z  ].C_V = 0.0001;
                    L[x+1][y  ][z+1].C_V = 0.0001;
                    L[x  ][y+1][z+1].C_V = 0.0001;
                }
...
double gammaAV(int ai, int aj, int ak, int ni, int nj, int nk) {
    return NYU_0 * exp( -(E_SADDLE - energyAVBefore(ai, aj, ak, ni, nj, nk) + addElectricField(ai, aj, ak, ni, nj, nk)) / (T*K_B) );
}


// The probability of an exchange of atoms (j, i) per unit time
double gammaVA(int ai, int aj, int ak, int ni, int nj, int nk) {
    return NYU_0 * exp( -(E_SADDLE - energyVABefore(ai, aj, ak, ni, nj, nk) + addElectricField(ai, aj, ak, ni, nj, nk)) / (T*K_B) );
}
...
double bindingEnergy(int xi, int yj, int zk) {
    ...
    return V_AA/E_V * neighborsSum;    // (eV)
}
...
double addElectricField(int ax, int ay, int az, int nx, int ny, int nz) {
    return (Z_A * E_V * (L[ax][ay][az].phiOld - L[nx][ny][nz].phiOld)) / E_V;
}
