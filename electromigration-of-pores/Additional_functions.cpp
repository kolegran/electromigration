...
void initFromFile(void) {
    ...
    fIn = fopen("000000000.txt", "r"); 
    
    // Check file opening
    if (fIn) {

        N = fscanf(fIn, "%d\n\n", &N);        // Record number of sites

        // Fill the array with data from a file
        for (x = 0; x < 2*NX; x += 2) {
            for (y = 0; y < 2*NY; y += 2) {
                for (z = 0; z < 2*NZ; z += 2){
                    fscanf(fIn, "%d\t%d\t%d\t%lf\n", &i, &j, &k, &L[x  ][y  ][z  ].C_V);
                    fscanf(fIn, "%d\t%d\t%d\t%lf\n", &i, &j, &k, &L[x+1][y+1][z  ].C_V);
                    fscanf(fIn, "%d\t%d\t%d\t%lf\n", &i, &j, &k, &L[x  ][y+1][z+1].C_V);
                    fscanf(fIn, "%d\t%d\t%d\t%lf\n", &i, &j, &k, &L[x+1][y  ][z+1].C_V);
                    ...
                    sites += 4;               // For check the number of sites            
                }
            }
        }
        // Check periodic boundary conditions
        for (x = 0; x < 2*NX; x++) {
            for (y = 0; y < 2*NY; y++) {
                for (z = 0; z < 2*NZ; z++) {
                    if (!((x+y+z) & 01)) {    // Checking the parity condition for FCC (use to shorten the code)
                        neighborsOfAtomPBC(x, y, z);
                    }
                }
            }
    ...
    // Output number of sites
    printf("Read %d sites done!\n", sites);
